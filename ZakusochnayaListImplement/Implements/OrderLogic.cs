...
namespace ZakusochnayaListImplement.Implements
{
    public class OrderLogic : IOrderLogic
    {
        ...
        {
            Order.BludoId = model.BludoId == 0 ? Order.BludoId : model.BludoId;
        ...
        private OrderViewModel CreateViewModel(Order Order)
        {
            ...
            for (int j = 0; j < source.Bludos.Count; ++j)
            {
                if (source.Bludos[j].Id == Order.BludoId)
                {
                    ForgeProductName = source.Bludos[j].BludoName;
                    break;
                }
            }
